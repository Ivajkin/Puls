...
class MetaImageSlide extends MetaSlide {
    ...
    protected function get_admin_slide() {
        ...
        // slide row HTML
        $row  = "<tr class='slide image flex responsive nivo coin bx'>";
        $row .= "    <td class='col-1'>";
    ...
    protected function get_public_slide() {
        ...
        case "bx":
            return $this->get_bx_slides_markup( $slide );
        default:
            return $this->get_flex_slider_markup( $slide );
        }
    }
     ...
     * @return string slide html
     */
    private function get_bx_slides_markup( $slide ) {
        $attributes = apply_filters( 'metaslider_bx_slider_image_attributes', array(
            'src' => $slide['src'],
            'height' => $slide['height'],
            'width' => $slide['width'],
            'alt' => $slide['alt'],
        ...
        $thumb = isset( $slide['data-thumb'] ) && strlen( $slide['data-thumb'] ) ? " data-thumb=\"{$slide['data-thumb']}\"" : "";
        $img = $this->build_image_tag( $attributes );

        $anchor_attributes = apply_filters( 'metaslider_bx_slider_anchor_attributes', array(
            'href' => $slide['url'],
            'target' => $slide['target']
        ), $slide, $this->slider->ID );

        if ( strlen( $anchor_attributes['href'] ) ) {
            $img = $this->build_anchor_tag( $anchor_attributes, $img );
        }
        ...
        $html = "<div class='img' $thumb>" . $img . "</div>" . $html;

        return apply_filters( 'metaslider_image_bx_slider_markup', $html, $slide, $this->settings );
    }
