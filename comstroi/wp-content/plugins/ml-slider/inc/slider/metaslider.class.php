...
class MetaSlider {
    ...
    private function get_settings() {
        ...
        if ( is_array( $settings ) &&
            isset( $settings['type'] ) &&
            in_array( $settings['type'], array( 'flex', 'coin', 'nivo', 'responsive', 'bx' ) ) ) {
            return $settings;
        } else {
...
EOT
    ...
    public function render_public_slides() {
        $html[] = '<!-- meta slider -->';
        $html[] = '<div class="' . $this->get_container_class() .'">';
        $html[] = '    ' . $this->get_inline_css();
        $html[] = '    <div id="' . $this->get_container_id() . '" class="slider clearfix">';
        $html[] = '        ' . $this->get_html();
    ...
    private function get_inline_javascript() {
        ...
        switch ($this->js_function) {
            case 'bxSlider':
                $tmp= explode(':"', $this->get_javascript_parameters());
                $sets = rtrim(str_replace('bx_width', '', $tmp[1]), " \t\n\r\0\x0B\",");
                $wadj = rtrim($tmp[2], " \t\n\r\0\x0B\"");
...
onSliderLoad: function (curIndex) {
        if (!$('.bx-controls-direction').hasClass('invisible'))
            $('.bx-controls-direction').toggleClass('simple invisible');
        $('.slider-viewport')
...
if ($('.bxslider').length) {
    bx_slider = $('#$identifier').bxSlider(sets);
    $(window).resize(function () {
                        mw = $('.slider').width();
                        tmp= $('body').width();
                        //if (tmp >= mediawidthmin && tmp < mediawidthmax) {
                        sets.slideMargin = crule(slmargin, mw);
                        sets.slideWidth = crule(slwidth, mw);
                        bx_slider.reloadSlider(sets);
                        //}
                    });
}
EOT;
