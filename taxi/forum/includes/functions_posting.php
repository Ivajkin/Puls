...
{
	global $db, $auth, $user, $config, $phpEx, $template, $phpbb_root_path, $ct_result;
...
// Begin : cleantalk.ru moderation
	...
	if (isset($ct_result) && $ct_result->allow == 0)
	{
		$post_approval = 0;
		if (isset($ct_result->comment))
		{
			$data['message'] = $data['message'] . "<br /><br />" . $ct_result->comment;
		}
	}
	// Force approve for message if cleantalk.ru allow it
        if (isset($ct_result) && $ct_result->allow == 1 && !$post_approval)
	{
		$post_approval = 1;
	}
