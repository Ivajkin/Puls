...
<?php } ?><!-- begin mod: User reputation points --><?php if (sizeof($this->_tpldata['reputation_row'])) {  ?>

	<h3><?php echo ((isset($this->_rootref['L_RP_RECENT_POINTS'])) ? $this->_rootref['L_RP_RECENT_POINTS'] : ((isset($user->lang['RP_RECENT_POINTS'])) ? $user->lang['RP_RECENT_POINTS'] : '{ RP_RECENT_POINTS }')); ?></h3>
	<table class="table1" cellspacing="0">
		<thead>
		<tr>
			<th class="name"><?php echo ((isset($this->_rootref['L_RP_POINTS'])) ? $this->_rootref['L_RP_POINTS'] : ((isset($user->lang['RP_POINTS'])) ? $user->lang['RP_POINTS'] : '{ RP_POINTS }')); ?></th>
			<th class="name"><?php echo ((isset($this->_rootref['L_FROM'])) ? $this->_rootref['L_FROM'] : ((isset($user->lang['FROM'])) ? $user->lang['FROM'] : '{ FROM }')); ?></th>
			<th class="name"><?php echo ((isset($this->_rootref['L_RP_COMMENTS'])) ? $this->_rootref['L_RP_COMMENTS'] : ((isset($user->lang['RP_COMMENTS'])) ? $user->lang['RP_COMMENTS'] : '{ RP_COMMENTS }')); ?></th>
			<th class="name"><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?></th>
		</tr>
		</thead>
		<tbody>
	<?php $_reputation_row_count = (isset($this->_tpldata['reputation_row'])) ? sizeof($this->_tpldata['reputation_row']) : 0;if ($_reputation_row_count) {for ($_reputation_row_i = 0; $_reputation_row_i < $_reputation_row_count; ++$_reputation_row_i){$_reputation_row_val = &$this->_tpldata['reputation_row'][$_reputation_row_i]; ?>

		<tr class="<?php if (!($_reputation_row_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } ?>">
			<td><span><?php echo $_reputation_row_val['POINT_IMG']; ?></span></td>
			<td><span><?php echo $_reputation_row_val['FROM']; ?></span></td>
			<td><span><?php echo $_reputation_row_val['COMMENT']; ?></span></td>
			<td><span><?php if ($_reputation_row_val['POST_SUBJECT']) {  ?><a href="<?php echo $_reputation_row_val['U_POST']; ?>"><?php echo $_reputation_row_val['POST_SUBJECT']; ?></a><?php } else { echo ((isset($this->_rootref['L_RP_NA'])) ? $this->_rootref['L_RP_NA'] : ((isset($user->lang['RP_NA'])) ? $user->lang['RP_NA'] : '{ RP_NA }')); } ?></span></td>
		</tr>
	<?php }} ?>

		</tbody>
	</table>
<?php } ?><!-- end mod: User reputation points -->
