...
if ($submit || $preview || $refresh)
{
	...
	if ($config['ct_enable'] && ($mode == 'reply' || $mode == 'quote' || $mode == 'post') && !$preview && !sizeof($error))
	{
		...
		if ($moderate)
		{
                    ...
                    $submit_time = null;
                    if (isset($_SESSION['ct_submit_time']) && preg_match("/^\d+$/", $_SESSION['ct_submit_time']))
                        $submit_time = time() - $_SESSION['ct_submit_time'];
                    ...
                    if ($ct_result->allow == 0 && !empty($ct_result->blacklisted))
                    {
...
// Begin : cleantalk.ru moderation
                        if (!empty($ct_cleantalk_id)) {
                            $sql = 'UPDATE '.POSTS_TABLE.' SET ct_request_id="'.$ct_cleantalk_id.'" WHERE post_id="'.$data['post_id'].'"';
                            $db->sql_query($sql);
                        }
