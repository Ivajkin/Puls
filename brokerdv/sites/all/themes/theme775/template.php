...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme775
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme775_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme775_html_head_alter(&$head_elements) {
 ...
 */
function theme775_preprocess_search_block_form(&$vars) {
 ...
 */
function theme775_preprocess(&$vars, $hook) {
  $vars['theme775_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme775_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme775_doctype();
  $vars['rdf'] = _theme775_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme775_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme775_id_safe('section-' . $section);
 ...
 */
function theme775_preprocess_page(&$vars) {
 ...
 */
function theme775_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme775') . '/css/maintenance-page.css');
}
 ...
 */
function theme775_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme775_preprocess_block(&$vars, $hook) {
 ...
 */
function theme775_menu_tree($vars) {
 ...
 */
function theme775_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme775_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme775_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme775_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme775_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme775_breadcrumb_separator');
 ...
 * @return bool
 */
function theme775_tabs_float() {
  $float = (bool) theme_get_setting('theme775_tabs_float');
  $float_node = (bool) theme_get_setting('theme775_tabs_node');
...
function theme775_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme775_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme775_id_safe($element['#title']);
 ...
 */
function theme775_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme775_menu_local_tasks(&$vars) {
  $output = '';
